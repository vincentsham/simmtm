...
from models.loss import l2_reg_loss, contrastive_loss
...
class SupervisedRunner(BaseRunner):
    ...
    def evaluate(self, epoch_num=None, keep_all=True):
        ...
        if self.classification:
            ...
            predictions = torch.argmax(probs, dim=1).detach().cpu().numpy()  # (total_samples,) int class index for each sample
...
class SimMTMRunner(BaseRunner):
    ...
    def train_epoch(self, epoch_num=None):
        ...
        for i, batch in enumerate(self.dataloader):
            ...
            predictions, R, w1, w2 = self.model(targets)  # (batch_size, padded_length, feat_dim)

            # Cascade noise masks (batch_size, padded_length, feat_dim) and padding masks (batch_size, padded_length)
            # target_masks = target_masks * padding_masks.unsqueeze(-1)
            
            mse_loss = self.loss_module(predictions, targets)  # (num_active,) individual loss (square error per element) for each active value in batch
            # batch_loss = torch.sum(loss)
            # mean_loss = batch_loss / len(loss)  # mean loss (over active elements) used for optimization
            loss_con = contrastive_loss(R, targets.shape[0])

            total_loss = 1/(w1.pow(2)) * mse_loss + 1/(w2.pow(2)) * loss_con + torch.log(w1) + torch.log(w2)
            ...
            metrics = {"mse loss": mse_loss.item(), "con loss": loss_con.item(), "w1": w1.item(), "w2": w2.item(), "total": total_loss.item()}
            if i % self.print_interval == 0:
                ending = "" if epoch_num is None else 'Epoch {} '.format(epoch_num)
                self.print_callback(i, metrics, prefix='Training ' + ending)
    ...
    def evaluate(self, epoch_num=None, keep_all=True):
        ...
        for i, batch in enumerate(self.dataloader):
            ...
            if keep_all:
                ...
                per_batch['targets'].append(targets.detach().cpu().numpy())
                per_batch['predictions'].append(predictions.detach().cpu().numpy())
                per_batch['metrics'].append([total_loss.detach().cpu().numpy()])
                per_batch['IDs'].append(IDs)
