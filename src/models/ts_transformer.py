...
from datasets.dataset import geom_noise_mask_single


def model_factory(config, data):
    task = config['task']
    if len(data.feature_df.shape) > 1:
    ...
    if (task == "imputation") or (task == "transduction"):
        if config['model'] == 'LINEAR':
            ...
            return TSTransformerEncoder(feat_dim, max_seq_len, config['d_model'], config['num_heads'],
                                        ...
                                        norm=config['normalization_layer'], freeze=config['freeze'])
    if task == "simmtm":
        return SimMTMTransformerEncoder(feat_dim, max_seq_len, config['d_model'], config['num_heads'],
                                        config['num_layers'], config['dim_feedforward'], config['out_len'],
                                        config['out_dim'], dropout=config['dropout'],
                                        pos_encoding=config['pos_encoding'], activation=config['activation'],
                                        norm=config['normalization_layer'], freeze=config['freeze'])
...
class TSTransformerEncoder(nn.Module):
    ...
    def forward(self, X, padding_masks=None):
        ...
        if type(padding_masks) != type(None):
...
class SimMTMTransformerEncoder(nn.Module):
    ...
    def __init__(self, feat_dim, max_len, d_model, n_heads, num_layers, dim_feedforward, out_len, out_dim, dropout=0.05,
                 pos_encoding='fixed', activation='gelu', norm='BatchNorm', freeze=False, temporal_unit=3):
        ...
        self.pos_enc1 = get_pos_encoder(pos_encoding)(d_model, max_len=max_len)
        self.pos_enc2 = get_pos_encoder(pos_encoding)(d_model, max_len=out_len)
    ...
    def forward(self, X):
        ...
        # permute because pytorch convention for transformers is [seq_length, batch_size, feat_dim]. padding_masks [batch_size, feat_dim]
        _x = X
        # _mask = torch.ones(X.shape[0], X.shape[1], X.shape[2]).to(X.device)
        for i in range(self.temporal_unit):
            # mask = np.random.binomial(1, 0.5, X.shape[0] * X.shape[1] * X.shape[2])
            # print(mask)
            ...
            # _mask = torch.cat([_mask, mask], axis=-1) # [batch_size, seq_length, feat_dim * temporal_unit]
        
        _x = _x.reshape(X.shape[0] * (self.temporal_unit + 1), X.shape[1], X.shape[2])
        
        # _mask = _mask.reshape(X.shape[0] * (self.temporal_unit + 1), X.shape[1], X.shape[2]).to(dtype=torch.bool)
        ...
        output = self.act(output)  # the output transformer encoder/decoder embeddings don't include non-linearity
        output = output.permute(1, 0, 2)  # (batch_size, seq_length, d_model)
        z = self.dropout1(output)
        # import ipdb; ipdb.set_trace()

        z_hat, R, norm = self.project(z, self.tau)
        # Most probably defining a Linear(d_model,feat_dim) vectorizes the operation over (seq_length, batch_size).
        output = self.output_layer(z_hat)  # (batch_size, seq_length, feat_dim)
        

        return output, R, self.w1, self.w2, norm
    ...
    def project(self, z, tau):
        ...
        # print("qweqweqw", v.shape, norm_v.shape, u.shape, norm_u.shape, torch.matmul(v,u).shape)
        R = torch.matmul(v,u)
        
        # print("Before", R.shape)
        _R = R = torch.exp(R/tau) # (batch + mask size) x (batch + mask size)
        # R = R.fill_diagonal_(0)
        R = R * (torch.ones_like(R) - torch.eye(R.shape[0], device=R.device)) # zero out the weight of no masked component
        # print("middle 1", R, R.shape, R.sum(-1).isnan().any(), (R > np.exp(1.1)).sum() )
        # print("middle", R, R.shape)
        R = R/R.sum(-1).unsqueeze(-1)
        # print("middle 2", R.shape, R.sum(-1).mean())
        M = self.temporal_unit + 1
        R = R[::M] # extract every no mask unit # (batch size) x (batch + mask size)
        # print("final", R.shape, z.shape)
        # import ipdb; ipdb.set_trace()
        z_hat = (R.unsqueeze(-1).unsqueeze(-1) * z.unsqueeze(0)).sum(1) 
        return z_hat, _R, norm_v

    def predict(self, X, padding_masks=None):
        """
        Args:
            X: (batch_size, seq_length, feat_dim) torch tensor of masked features (input)
            padding_masks: (batch_size, seq_length) boolean tensor, 1 means keep vector at this position, 0 means padding
...
#     def __init__(self, feat_dim, max_len, d_model, n_heads, num_layers, dim_feedforward, out_len, out_dim, dropout=0.0,
#                  pos_encoding='fixed', activation='gelu', norm='BatchNorm', freeze=False, temporal_unit=1):
#         super(SimMTMTransformerEncoder, self).__init__()
...
#             if norm == 'LayerNorm':
#                 encoder_layer1 = TransformerEncoderLayer(1, 1, dim_feedforward, dropout, activation=activation)
#             else:
#                 encoder_layer1 = TransformerBatchNormEncoderLayer(1, 1, dim_feedforward, dropout, activation=activation)

#             self.transformer_encoder1.append(nn.TransformerEncoder(encoder_layer1, 1))

#         self.transformer_encoder2 = nn.ModuleList([])
#         for _ in range(num_layers):
#             if norm == 'LayerNorm':
#                 encoder_layer2 = TransformerEncoderLayer(d_model, self.n_heads, dim_feedforward, dropout, activation=activation)
#             else:
#                 encoder_layer2 = TransformerBatchNormEncoderLayer(d_model, self.n_heads, dim_feedforward, dropout, activation=activation)

#             self.transformer_encoder2.append(nn.TransformerEncoder(encoder_layer2, 1))
...
#             inp = inp.permute(2, 1, 0)  # (d_model, batch_size, seq_length)
#             inp = inp.reshape(self.d_model, -1, 1)
#             inp = self.transformer_encoder1[i](inp) 
#             inp = inp.reshape(self.d_model, _x.shape[0], _x.shape[1])
#             inp = inp.permute(2, 1, 0)   # (seq_length, batch_size, d_model)
#             inp = self.transformer_encoder2[i](inp) 
...
#             inp = inp.reshape(self.d_model, -1, 1)
#             inp = self.transformer_encoder1[i](inp) 
#             inp = inp.reshape(self.d_model, X.shape[0], X.shape[1])
#             inp = inp.permute(2, 1, 0)   # (seq_length, batch_size, d_model)
#             inp = self.transformer_encoder2[i](inp) 
